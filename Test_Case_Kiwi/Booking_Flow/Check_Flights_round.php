    ...
    for ($attempt = 1; $attempt <= $max_attempts; $attempt++) {
        ...
        if (
            ...
            if (isset($_SESSION['flight_result']['data']) && is_array($_SESSION['flight_result']['data'])) {
                foreach ($_SESSION['flight_result']['data'] as $search_flight_offer) {
                    if (isset($search_flight_offer['booking_token']) && $search_flight_offer['booking_token'] === $bookingToken) {
                        // The 'route' from search results has all segments for the offer
            ...
            if ($original_detailed_route) {
                // The check_flights API response often has a less detailed 'flights' array.
                // We replace it with the detailed 'route' from the search results.
                // flight_details_round.php will use this array.
                $session_payload['flights'] = $original_detailed_route;
                $rsLogger->info("Merged original detailed route into session payload for booking token: $bookingToken");
            } else {
                 // Fallback to use the route from the check_flights response if available.
                 if(isset($data['flights'][0]['route'])) {
                     $session_payload['flights'] = $data['flights'][0]['route'];
                 }
                $rsLogger->warning("Original detailed route not found in search_results for booking token: $bookingToken. Using route from check_flights API if available.");
            }
        
            // Store the (potentially enhanced) data array in session
            $_SESSION['data'] = $session_payload; 
        
            // Redirect to flight_details_round.php as it was originally intended.
            header('Location: /Test/views/flight_details_round.php');
            exit;
        } 
    }}

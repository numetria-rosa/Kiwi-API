...
$departure_date_raw = sanitizeInput($_GET['departure_date'] ?? "");
$return_date_raw = sanitizeInput($_GET['return_date'] ?? "");

$_SESSION['departure_date'] = $departure_date_raw;
$_SESSION['return_date'] = $return_date_raw;

$date_from    = convertDateToAPIFormat($departure_date_raw);
$date_to      = $date_from;
$return_from  = convertDateToAPIFormat($return_date_raw);
...
$cabin_class      = sanitizeInput($_GET['cabin_class'] ?? "");
$mix_with_cabins  = "";

$adult_hold_bag   = sanitizeInput($_GET['adult_hold_bag'] ?? "");
...
echo "<pre>";
echo "Sanitized input values:\n";
echo "fly_from: ";         var_dump($fly_from);
echo "fly_to: ";           var_dump($fly_to);
echo "date_from: ";        var_dump($date_from);
echo "date_to: ";          var_dump($date_to);
echo "return_from: ";      var_dump($return_from);
echo "return_to: ";        var_dump($return_to);
echo "adults: ";           var_dump($adults);
echo "children: ";         var_dump($children);
echo "infants: ";          var_dump($infants);
echo "cabin_class: ";      var_dump($cabin_class);
echo "mix_with_cabins: ";  var_dump($mix_with_cabins);
echo "adult_hold_bag: ";   var_dump($adult_hold_bag);
echo "adult_hand_bag: ";   var_dump($adult_hand_bag);
echo "child_hold_bag: ";   var_dump($child_hold_bag);
echo "child_hand_bag: ";   var_dump($child_hand_bag);
echo "\n\nRaw \$_GET values:\n";
print_r($_GET);
echo "</pre>";


function SearchFlightRoundTrip(
    ...
    // Log Request
    $requestLogger = new Logger('RQ_SearchFlightRoundTrip');
    $requestLogger->pushHandler(new StreamHandler('C:/xampp/htdocs/Test_Case_Kiwi/Queues/Searchs/Round/RQ_SearchFlightRoundTrip.txt', Logger::INFO));
    ...
    // Log Response
    $responseLogger = new Logger('RS_SearchFlightRoundTrip');
    $responseLogger->pushHandler(new StreamHandler('C:/xampp/htdocs/Test_Case_Kiwi/Queues/Searchs/Round/RS_SearchFlightRoundTrip.txt', Logger::INFO));
...
// echo '<h2 style="text-align:center;">Search Results (Round Trip)</h2>';
// echo '<p style="text-align:center;"><strong>Search ID:</strong> ' . htmlspecialchars($result['search_id']) . '</p>';

// foreach ($result['data'] as $flight) {
//     $_SESSION['session_id'] = $result['search_id'];
//     $_SESSION['booking_token'] = isset($flight['booking_token']) ? $flight['booking_token'] : null;
...
//         $checkFlightsLink = '../Check_Flights_original.php?session_id=' . urlencode($sessionId) . '&booking_token=' . urlencode($bookingToken);
...
//                 <p><strong>From/To:</strong> ' . htmlspecialchars($flight['cityFrom']) . ' (' . htmlspecialchars($flight['flyFrom']) . ')</p>
//                 <p><strong>To/From:</strong> ' . htmlspecialchars($flight['cityTo']) . ' (' . htmlspecialchars($flight['flyTo']) . ')</p>
...
//                 // Assuming $flight is your decoded JSON object from the API response
...
//                 if ($returnFlight) {
//                     // Convert return flight times to DateTime and format them
//                     $returnDepartureDate = new DateTime($returnFlight['local_departure']);
//                     $returnArrivalDate = new DateTime($returnFlight['local_arrival']);
                
//                     $formattedReturnDeparture = $returnDepartureDate->format('l, F j, Y \a\t g:i A');
//                     $formattedReturnArrival = $returnArrivalDate->format('l, F j, Y \a\t g:i A');
                
//                     echo "<h2>Return Flight Information</h2>";
//                     echo "<p><strong>From:</strong> " . htmlspecialchars($returnFlight['cityFrom']) . " (" . htmlspecialchars($returnFlight['flyFrom']) . ")</p>";

//                     echo "<p><strong>To:</strong> " . htmlspecialchars($returnFlight['cityTo']) . " (" . htmlspecialchars($returnFlight['flyTo']) . ")</p>";
//                     echo "<p><strong>Airline:</strong> " . htmlspecialchars($returnFlight['airline']) . "</p>";
//                     echo "<p><strong>Flight Number:</strong> " . htmlspecialchars($returnFlight['flight_no']) . "</p>";
//                     echo "<p><strong>Departure:</strong> " . $formattedReturnDeparture . "</p>";
//                     echo "<p><strong>Arrival:</strong> " . $formattedReturnArrival . "</p>";
//                     echo "<p><strong>Duration:</strong> " . gmdate("H:i", $flight['duration']['return']) . " hours</p>";
//                 }
...
header("Location: ../../../Test/views/search_results_round.php");
exit;
